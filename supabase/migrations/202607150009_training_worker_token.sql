-- Add a second authorization layer because an apikey alone can pass the Edge gateway.
...
begin
  ...
  perform cron.schedule('leather-training-worker','*/5 * * * *',$job$
    select net.http_post(
      ...
      headers := jsonb_build_object('Content-Type','application/json','Authorization','Bearer ' || (select decrypted_secret from vault.decrypted_secrets where name='training_publishable_jwt'),'apikey',(select decrypted_secret from vault.decrypted_secrets where name='training_publishable_jwt'),'x-worker-secret',(select decrypted_secret from vault.decrypted_secrets where name='training_worker_token')),
      body := '{}'::jsonb,timeout_milliseconds := 10000
    );
  $job$);
  perform cron.schedule('leather-training-cron-cleanup','17 3 * * 0',$job$ delete from cron.job_run_details where end_time < now()-interval '30 days'; $job$);
end $$;
