...
async function renderPublicProfile(id) {
  ...
  try {
    ...
    const [achievements, following, social] = await Promise.all([api.fetchUserAchievements(profile.id), api.isFollowingUser(profile.id), api.fetchProfileSocial(profile.id)]);
...
function profileSocialList(title, users = []) {
  const items = users.slice(0, 60).map(user => `<a class="profile-social-item" href="#profile/${encodeURIComponent(user.id)}">${avatarHtml(user)}<span><b class="name-${user.role === "user" ? user.name_color : "purple"}">${esc(user.display_name)}</b><small>@${esc(user.handle)} · ${nowText(Date.parse(user.followed_at))}</small></span></a>`).join("");
  return `<section class="profile-social-section"><div class="comment-title"><div><span class="eyebrow">SOCIAL</span><h2>${esc(title)}</h2></div><span>${users.length} 人</span></div><div class="profile-social-list">${items || '<div class="empty-list">暂无用户。</div>'}</div></section>`;
}
...
function adminContentLink(table, item) {
